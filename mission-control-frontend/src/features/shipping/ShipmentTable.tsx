...
import { MdEdit, MdLocationOn, MdCalendarToday, MdLocalShipping } from "react-icons/md";
  ...
  return (
    <div className="w-full divide-y divide-border">
      {shipments.map((shipment) => (
        <div
          key={shipment.id}
          className="flex w-full flex-col gap-2 bg-gray-900/25 p-3 hover:bg-slate-800/25 md:gap-4 md:p-6 lg:p-8"
        >
          <div className="flex w-full items-start justify-between gap-2">
            <div className="flex w-full flex-col gap-2 md:gap-3">
              {/* Header with shipment ID and type */}
              <div className="flex flex-wrap items-center gap-1.5 md:gap-2">
                <h3 className="text-base font-semibold opacity-85 sm:text-lg md:text-xl lg:text-2xl">
                  {shipment.shipmentId}
                </h3>
                {/* <span className="rounded-full bg-blue-500/20 px-2 py-0.5 text-xs text-blue-400">
                  {shipment.type === "robot" ? "Robot Shipping" : "Miscellaneous"}
                </span> */}
                <span
              ...
              {shipment.type === "robot" && shipment.robots && (
                <div className="flex flex-wrap gap-1 md:gap-1.5">
                  {shipment.robots.slice(0, 3).map((robot, idx) => (
                    <span
                      key={idx}
                      className="rounded bg-slate-700/50 px-1.5 py-0.5 text-md text-neutral-300 md:px-2"
                    >
                      {robot.name}
                    </span>
                  ))}
              ...
              <Accordion type="single" collapsible className="mt-1 md:mt-2">
                ...
                >
                  <AccordionTrigger className="px-2 py-2 hover:no-underline md:px-3 md:py-3">
